...
[ExecuteInEditMode]
public class IK_toolkit_UR5 : MonoBehaviour
{
    public Transform ik;
    ...
    public static double[,] DH_matrix_UR5 = new double[6, 3] {
    ...
    public static Matrix4x4 ComputeTransformMatrix(int jointIndex, double[,] jointAngles)
    {
        ...
        translationZ.m23 = (float)DH_matrix_UR5[jointIndex, 2];

        // Translation along the X-axis
        var translationX = Matrix4x4.identity;
        translationX.m03 = (float)DH_matrix_UR5[jointIndex, 0];

        // Rotation around the X-axis
        var rotationX = Matrix4x4.identity;
        rotationX.m11 = Mathf.Cos((float)DH_matrix_UR5[jointIndex, 1]);
        rotationX.m12 = -Mathf.Sin((float)DH_matrix_UR5[jointIndex, 1]);
        rotationX.m21 = Mathf.Sin((float)DH_matrix_UR5[jointIndex, 1]);
        rotationX.m22 = Mathf.Cos((float)DH_matrix_UR5[jointIndex, 1]);
    ...
    public static double[,] Inverse_kinematic_solutions(Matrix4x4 transform_matrix_unity)
    {
        ...
        Vector4 P05 = transform_matrix_unity * new Vector4()
        {
            x = 0,
            y = 0,
            z = -(float)DH_matrix_UR5[5, 2],
            w = 1
        }; ;
        float psi = Mathf.Atan2(P05[1], P05[0]);
        float phi = Mathf.Acos((float)((DH_matrix_UR5[1, 2] + DH_matrix_UR5[3, 2] + DH_matrix_UR5[2, 2]) / Mathf.Sqrt(Mathf.Pow(P05[0], 2) + Mathf.Pow(P05[1], 2))));
        ...
        for (int i = 0; i < 8; i += 4)
        {
            double t5 = (transform_matrix_unity[0, 3] * Mathf.Sin((float)theta[0, i]) - transform_matrix_unity[1, 3] * Mathf.Cos((float)theta[0, i]) - (DH_matrix_UR5[1, 2] + DH_matrix_UR5[3, 2] + DH_matrix_UR5[2, 2])) / DH_matrix_UR5[5, 2];
            float th5;
            if (1 >= t5 && t5 >= -1)
            {
        ...
        for (int i = 0; i <= 7; i += 2)
        {
            ...
            {
                x = 0,
                y = (float)-DH_matrix_UR5[3, 2],
                z = 0,
                w = 1
            };
            double t3 = (Mathf.Pow(P13[0], 2) + Mathf.Pow(P13[1], 2) - Mathf.Pow((float)DH_matrix_UR5[1, 0], 2) - Mathf.Pow((float)DH_matrix_UR5[2, 0], 2)) / (2 * DH_matrix_UR5[1, 0] * DH_matrix_UR5[2, 0]);
            double th3;
            if (1 >= t3 && t3 >= -1)
            {
        ...
        for (int i = 0; i < 8; i++)
        {
            ...
            theta[1, i] = Mathf.Atan2(-P13[1], -P13[0]) - Mathf.Asin((float)(-DH_matrix_UR5[2, 0] * Mathf.Sin((float)theta[2, i]) / Mathf.Sqrt(Mathf.Pow(P13[0], 2) + Mathf.Pow(P13[1], 2))));
